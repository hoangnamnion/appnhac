...
class App {
  ...
  bindEvents() {
    ...
    if (btnDownloadAll) {
      btnDownloadAll.addEventListener('click', () => this.downloadAllOffline());
    }
  ...
  /**
   * Download a single track audio blob from cloud and save to IndexedDB for offline playback
   */
  async downloadTrackOffline(track) {
    ...
    try {
      ...
      await StorageEngine.saveTrack(track);
      const idx = this.currentTrackList.findIndex(t => t.id === track.id);
      if (idx !== -1) this.currentTrackList[idx].audioBlob = blob;
      this.showToast(`✅ "${track.title}" đã lưu offline!`);
      return true;
    } catch (err) {
      console.error('Download offline failed:', err);
      this.showToast(`❌ Tải thất bại: ${err.message}`);
      return false;
    }
  }

  /**
   * Download ALL cloud-only tracks to IndexedDB for offline playback
   */
  async downloadAllOffline() {
    const cloudOnly = this.currentTrackList.filter(t => !t.audioBlob && t.audioUrl);
    if (cloudOnly.length === 0) {
      this.showToast('✅ Tất cả bài hát đã ở chế độ offline rồi!');
      return;
    }
    const btn = document.getElementById('btnDownloadAll');
    if (btn) btn.disabled = true;
    for (let i = 0; i < cloudOnly.length; i++) {
      const track = cloudOnly[i];
      this.showToast(`⬇️ Đang tải ${i + 1}/${cloudOnly.length}: "${track.title}"...`);
      await this.downloadTrackOffline(track);
    }
    if (btn) btn.disabled = false;
    this.renderLibraryList(this.currentTrackList);
    this.showToast(`🎉 Đã tải về ${cloudOnly.length} bài! Nghe offline được rồi.`);
  }
  ...
  renderLibraryList(tracks) {
    ...
    tracks.forEach((track, index) => {
      ...
      // --- Offline Status / Save-to-Device Button ---
      const isOffline = !!(track.audioBlob);
      const offlineBtn = document.createElement('button');
      offlineBtn.className = 'ios-circle-btn btn-offline-track';
      offlineBtn.style.width = '36px';
      offlineBtn.style.height = '36px';
      offlineBtn.dataset.trackId = track.id;

      if (isOffline) {
        offlineBtn.title = '\u2705 \u0110\u00e3 l\u01b0u offline';
        offlineBtn.style.color = 'var(--ios-green)';
        offlineBtn.innerHTML = '<svg viewBox="0 0 24 24"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>';
        offlineBtn.disabled = true;
      } else if (track.audioUrl) {
        offlineBtn.title = '\u2601\ufe0f T\u1ea3i v\u1ec1 m\u00e1y (Offline)';
        offlineBtn.style.color = 'var(--ios-blue)';
        offlineBtn.innerHTML = '<svg viewBox="0 0 24 24"><path d="M5 20h14v-2H5v2zm7-18v10.17l-3.59-3.58L7 10l5 5 5-5-1.41-1.41L13 12.17V2h-1z"/></svg>';
        offlineBtn.addEventListener('click', async (e) => {
          e.stopPropagation();
          offlineBtn.disabled = true;
          offlineBtn.innerHTML = '<svg viewBox="0 0 24 24" style="animation:spin 1s linear infinite"><path d="M12 4V1L8 5l4 4V6c3.31 0 6 2.69 6 6h2c0-4.42-3.58-8-8-8z"/></svg>';
          const success = await this.downloadTrackOffline(track);
          if (success) {
            offlineBtn.title = '\u2705 \u0110\u00e3 l\u01b0u offline';
            offlineBtn.style.color = 'var(--ios-green)';
            offlineBtn.innerHTML = '<svg viewBox="0 0 24 24"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>';
        ...
        offlineBtn.title = 'Kh\u00f4ng c\u00f3 ngu\u1ed3n audio';
        offlineBtn.style.color = 'var(--text-tertiary)';
        offlineBtn.innerHTML = '<svg viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>';
        offlineBtn.disabled = true;
      }
      actionsDiv.appendChild(offlineBtn);

      // --- Export to File Button (📤) ---
      const exportBtn = document.createElement('button');
      exportBtn.className = 'ios-circle-btn btn-export-track';
      exportBtn.title = 'Xuất file MP3 ra thiết bị';
      exportBtn.style.width = '36px';
      exportBtn.style.height = '36px';
      exportBtn.innerHTML = '<svg viewBox="0 0 24 24"><path d="M9 16h6v-6h4l-7-7-7 7h4v6zm-4 2h14v2H5v-2z"/></svg>';
      exportBtn.addEventListener('click', async (e) => {
        e.stopPropagation();
        const filename = `${track.artist || 'Unknown'} - ${track.title || 'Track'}.mp3`;
        
        let blobToSave = track.audioBlob;
        if (!blobToSave && track.audioUrl) {
          this.showToast('⏳ Đang tải file từ Cloud để lưu...');
          try {
            const resp = await fetch(track.audioUrl);
            blobToSave = await resp.blob();
          } catch (err) {
            console.error('Fetch cloud audio for save failed:', err);
          }
        }

        if (blobToSave) {
          const res = await NativeBridge.saveAudioFile(filename, blobToSave);
          if (res && res.success) {
            this.showToast(`💾 Đã xuất file: ${filename}`);
            NativeBridge.sendNotification('ShinTag Music', `Đã xuất "${filename}" thành công!`);
          } else if (res && res.canceled) {
            // User canceled save dialog
          } else {
            this.showToast(`❌ Không thể xuất file`);
          }
        } else {
          this.showToast('Không tìm thấy file audio để xuất!');
        }
      });
      ...
      row.addEventListener('click', (e) => {
        if (e.target.closest('.btn-offline-track') || e.target.closest('.btn-export-track') || e.target.closest('.btn-delete-track')) return;
